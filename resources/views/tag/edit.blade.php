    ...
    <div class="flex min-h-full items-center justify-center py-12 px-4 sm:px-6 lg:px-12">
        <div class="w-full space-y-8 max-w-sm md:max-w-md lg:max-w-lg">
            <div>
                ...
                <h2 class="mt-6 text-center text-3xl font-bold tracking-tight text-gray-900">Update Tag</h2>
                <p class="mt-2 text-center text-sm text-gray-600" />
            </div>
            <form class="mt-8 space-y-6" action="/tag/{{ $tag->id }}/update" method="POST" autocomplete="off">
                @csrf
                @method('PUT')

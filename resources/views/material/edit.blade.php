    ...
    <div class="flex min-h-full items-center justify-center py-12 px-4 sm:px-6 lg:px-12">
        <div class="w-full space-y-8 max-w-sm md:max-w-md lg:max-w-lg">
            <div>
                ...
                <h2 class="mt-6 text-center text-3xl font-bold tracking-tight text-gray-900">Edit Material</h2>
                <p class="mt-2 text-center text-sm text-gray-600" />
            </div>
            <form class="mt-8 space-y-6" action="/material/{{$material->id}}/update" method="POST" autocomplete="off">
                @csrf
                @method("PUT")
                ...
                <div class="-space-y-px rounded-md shadow-sm">
                    <div>
                        ...
                        <textarea id="description" name="description" class="relative block w-full appearance-none rounded-none rounded-t-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" placeholder="Material Description" rows="5">{{ $material->description }}</textarea>
                    </div>
                    @error('description')
                        <p class="text-red-500 text-sm">{{ $message }}</p>
                    @enderror

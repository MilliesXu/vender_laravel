            ...
            <div class="flex flex-start gap-2 grow flex-wrap mb-2 md:mb-0">
                @foreach($material->tags as $tag)
                    <div class="pl-5 pr-3 rounded-lg text-white bg-gray-700 flex gap-x-2" id="{{ $tag->name }}_{{$tag->id}}">
                        ...
                        <button class="px-2 text-white hover:text-gray-300" data-bs-toggle="modal" data-bs-target="#modal_delete_tag_{{ $material->id }}_{{ $tag->id }}">
                            <i class="fa-solid fa-trash-can"></i>
                        </button>
                    </div>

                    <div class="relative z-10 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true" id="modal_delete_tag_{{ $material->id }}_{{ $tag->id }}">
                        <!--
                        ...
                        <div class="fixed inset-0 z-10 overflow-y-auto">
                            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                                ...
                                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all w-10/12">
                                    <form class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4" action="/material_tag/{{ $material->id }}/{{ $tag->id }}" method="post">
                                        @csrf
                                        @method('delete')
                                        <label for="countries" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Delete Tag</label>
                                        ...
                                        <div class="bg-gray-50 py-3 sm:flex sm:flex-row-reverse sm:px-6 md:px-0">
                                            ...
                                            <button type="button" class="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" data-bs-toggle="modal_close" data-bs-target="#modal_delete_tag_{{ $material->id }}_{{ $tag->id }}">Cancel</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
